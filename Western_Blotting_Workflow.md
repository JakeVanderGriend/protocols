...
a.	Dilute 100uL of O/N into 900uL 70% IO in an eppi tube, vortex, and transfer to a cuvette.
b.	Divide produced OD by 0.1 to get total OD of the original culture.
2.	Spin down 1mL of O/N culture into a 1.7mL tube (3min, 8,000xg*)
3.	Wash 2X with 1mL 1X TBS (1min, 8,000xg*)
4.	Resuspend washed cells in 1% SDS solution ((OD/6) * 1,000 = uL SDS to resuspend in)
a.	Remember to back-calculate from your dilution first!
5.	Allow cells to incubate at room temperature 5 minutes.
6.	Spindown cells to remove cell debris (1min, 8,000xg)
...
a.	Note: This is a stopping point if you are mass-prepping samples. Make sure tubes are well mixed, and then store @ -20˚ C. When ready to run gels, thaw and then proceed with Step 9.
...
a.	Prepare 1X Tris/Glycine/SDS “Running” buffer, set aside.
a.	 (Can do this while cells cook @ 95˚ C.)
b.	 Running 1-2 gels: Make 750mL 1X Running buffer: 75mL of 10x stock in 675mL of DI H2O
c.	Running 3-4 gels: Make 1250mL 1X Running buffer: 125mL of 10x stock in 1125mL of DI H2O
b.	Setup gel rig apparatus:
a.	Unpackage gel - remove green strip from base and remove comb (Select your gel depending on the size of your protein. Ex: VF_2432 is ~19 kDa, so use a 4-20% gel) (https://github.com/mjmlab/protocols/blob/master/sds-page.md#gel) 
b.	Set up gel in electrode assembly.
i.	If only one gel, use buffer dam
ii.	Make sure wells face inside of the unit
c.	Use green clamps to secure gels
d.	Place electrode assembly into tank
e.	Pour running buffer into tank starting by filling the electrode assembly
i.	Electrode assembly should fill up and not leak into tank (Wait a minute or two to make sure there are no leaks.
ii.	Wells should be filled with running buffer (Take a pipette tip and gently pipette running buffer into wells), also check if the loading walls are bent.
iii.	Once assembly is full, fill tank to the 2 gel line with running buffer
iv.	Note: If you are only running one gel, you need to have it in the back of the tank, prongs can’t be in the front.
...
a.	Get one of the transfer apparatuses (white and black swiss cheese looking thing), and place black side on the bench on a bunch of paper towels.
b.	Place on one black sponge
c.	Place on one piece of filter paper (it helps to add transfer buffer from one of the soak bins onto the filter paper to reduce bubbles forming under the gel.)
d.	Place on gel (remember that gel will be mirrored in the output blot) use roller to remove any bubbles. Dump on a little transfer buffer from one of the bins.
e.	Using forceps at the far right side and the smallest contact with a finger on the bottom left side, apply the membrane to the gel in one swift motion. DO NOT MOVE THE MEMBRANE AFTER IT IS DOWN.
f.	Add another piece of filter paper on top of membrane and roll to remove bubbles (Roll in short movements across and up and down across the top filter paper.)
g.	Add another sponge on top of that
h.	At any of the above steps, it can help to wet the components by dumping on transfer buffer.
...
8.	Black plate of sandwich faces black face of transfer setup, clear side faces the red side. Put the iceblock into the empty section. Fill rig with transfer buffer and close lid, you will need the transfer buffer from your bins. (Buffer can be kept in the 4C prior to use, probably want to fill the rig in the cold room cause it will be heavy)
...
a.	For testing transfer, clean the GelDoc Stain-free tray surface 2X with 70% EtOH
b.	Select ‘Stain-free’ gel, then select the imaging menu, and set ‘Activation time’ to None, and use the manual exposure time to match the gel exposure time.
c.	Make a DI H2O puddle, then lay the membrane down with protein side UP. Image, then immediately proceed with Step 11. 
...
10mL 1x TBS-T
0.05g Non-fat milk
XuL Anti-TARGET (Manufactuer forms have recommended dilutions)
2uL Anti-RpoA Mouse IgG - 500 µg/mL  ***Make sure primary is NOT the same species IgG
...
10mL 1x TBS-T
0.05g Non-fat milk
2uL LI-COR IRDye 800CW Goat anti-Species IgG 1000 µg/mL 
2uL LI-COR IRDye® 680RD Goat anti-Mouse IgG 1000 µg/mL
...
9. Rinse the membrane briefly with ultra pure water, then load into saran-wrap/foil packets and image on the 3rd floor LI-COR using the 700 channel. 
	a. In the past, 30s was enough to visualize samples, but you may need to change this depending on signal.
